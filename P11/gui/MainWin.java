...
public class MainWin extends JFrame {
    ...
    public MainWin(String MainWintitle) {
        ...
        Image createCustomerButtonIcon = null;
        try {
            createCustomerButtonIcon = ImageIO.read(getClass().getResource("/gui/createCustomerButtonIcon.png"));
        } catch (IOException i) {
            ;
        } finally {
            ...
            createCustomerButton.setIcon(new ImageIcon(createCustomerButtonIcon));
    ...
    public product.Serving onCreateServingClick() {
        ...
        if (containerForServing != null) {
            for (int i = 0; i < containerForServing.maxScoops(); i++) {
                ...
                if (scoopForServingList.size() > 0) {
                    ...
                    do {
                        ...
                        if (toppingMixInFlavorForServing != null) {
                            product.MixInAmount toppingMixInAmountForServing = (product.MixInAmount) JOptionPane
                                    .showInputDialog(
                                            this,
                            ...
                            product.MixIn mixInForScoop = new product.MixIn(toppingMixInFlavorForServing,
                                    toppingMixInAmountForServing);
    ...
    public void onCreateCustomerClick() {
        class createCustomerDialog extends JDialog {
            ...
            private JTextField phoneOfCustomer;

            createCustomerDialog(Frame aFrame) {
                super(aFrame, "Create Customer", true);
                setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
                canceled = true;
                setSize(width, height);
                setLayout(new GridBagLayout());
                ...
                constraintsLabel.gridy = 0;
                add(name, constraintsLabel);
                nameOfCustomer = new JTextField(20);
                ...
                phoneOfCustomer = new JTextField(20);
                phoneOfCustomer.setText("");
                constraints.gridx = 1;
                constraints.gridy = 1;
                constraints.weighty = 1;
                add(phoneOfCustomer, constraints);
    ...
    private void view(Screen screen) {
        ...
        if (screen == Screen.CUSTOMERS) {
            to_display_string_1 = "";
            to_display_string_2 = "";
            to_display_string_1 += "<HTML><h1>CUSTOMERS</h1><br/>";
            if (emporium.customers() != null)
                for (int i = 0; i < emporium.customers().length; i++) {
                    to_display_string_2 += String.format("<h2>%s</h2>", (emporium.customers()[i]).toString());
                }
            to_display_string_2 += "</HTML>";
            display.setText(to_display_string_1 + to_display_string_2);
        }
